...
<head>
	...
	<script src = 'node_modules/node_modules/web3/dist/web3.min.js'></script>  <!-- error veriyo xd -->
...
async function load_nft_content(){
				...
				document.getElementById("pric1").innerHTML = ((await price()) / 1000000000000000000) + "ETH" ;
			fetch(url)
.then(res => res.json())
.then((out) => {
	img.src  = out["image"];
...
console.log(address_final);
                ...
                <div class="col-md-12 col-lg-6 col-sm-12 col-12">
					<div class="product-summary">
						  ...
						  <div class="price">Ticket price:<label id = "pric1">Price loading...</label></div>
						  ...
						  <div class="cart-info clearfix">
								...
								<div class="button col-6 p-0">
								    <input type="text" id = "ticketamount" class="form-control" value= "Enter ticket amount...">
									<button type="submit" onclick="BuyTicket();"class="btncart">Buy tickets</button>
								</div>
...
function getaddress(){
    ...
    //address gelcek
    return "0xF1Fc1d0aB933d1a76Bb28e330BC848aF5a00F05F";
...
async function AuctionTime(){
    ...
    console.log(currentUnixTimestap);
    const endtime = await window.contract.methods.auctionEndTime().call();
    const tokenid = await getaddress();
    console.log(endtime);
    const address = await window.contract.methods.currentWinner().call();
    if(currentUnixTimestap > endtime){
        var newUrl = "/auction_delete_from_db.php"+"?tokenid="+tokenid+"&newowner="+address;
...
async function Buynft(){
	try{
		...
		const tokenid = await getaddress();
		var newUrl = "/delete_from_db.php"+"?tokenid="+tokenid+"&newowner="+address;
		window.location.assign(newUrl);
...
async function price(){
    return await window.contract.methods.ticketprice().call();
}

async function BuyTicket(){
 const ticketcount =  document.getElementById('ticketamount').value;
 const address = await getCurrentAccount();
 var dummy = await price();
 window.contract.methods.buyticket(await getaddress(),ticketcount).send({from: address , value: await window.contract.methods.ticketprice().call() * ticketcount });
